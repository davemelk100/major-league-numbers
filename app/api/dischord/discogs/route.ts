...
const USER_AGENT = "Dischord/1.0";
const DISCHORD_LABEL_ID = 244;
...
async function getLabelReleaseLookup(): Promise<Map<string, number>> {
  ...
  try {
    const firstPage = await fetchFromDiscogs(
      `/labels/${DISCHORD_LABEL_ID}/releases?page=1&per_page=100&sort=year&sort_order=asc`
    );
    ...
    for (let batchStart = 2; batchStart <= totalPages; batchStart += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        pages.map((p) =>
          fetchFromDiscogs(
            `/labels/${DISCHORD_LABEL_ID}/releases?page=${p}&per_page=100&sort=year&sort_order=asc`
          ).catch(() => null)
        )
      );
