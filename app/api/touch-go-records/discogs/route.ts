...
async function getLabelReleaseLookup(): Promise<Map<string, number>> {
  ...
  try {
    const firstPage = await fetchFromDiscogs(
      `/labels/${TOUCH_GO_LABEL_ID}/releases?page=1&per_page=100&sort=year&sort_order=asc`
    );
    ...
    for (let batchStart = 2; batchStart <= totalPages; batchStart += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        pages.map((p) =>
          fetchFromDiscogs(
            `/labels/${TOUCH_GO_LABEL_ID}/releases?page=${p}&per_page=100&sort=year&sort_order=asc`
          ).catch(() => null)
        )
      );
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("Touch & Go Discogs API error:", error);
