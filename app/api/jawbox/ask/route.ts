...
import { searchJawboxSources, type JawboxSourceDoc } from "@/lib/jawbox-knowledge";
import {
...
const SYSTEM_PROMPT = "You are an expert on Jawbox. Answer questions about artists, releases, history, and milestones. Be concise and grounded in facts. If unsure, say so. Never generate media content.";
...
function formatSourceContext(sources: JawboxSourceDoc[]): string {
...
export async function POST(req: Request) {
  try {
    ...
    const sources = searchJawboxSources(lastUserMessage, 6);
    ...
    console.error("[JAWBOX] API ask error:", error);
    return new Response(
      JSON.stringify({
