...
const USER_AGENT = "Jawbox/1.0";
const JAWBOX_LABEL_ID = 2921;
...
async function getLabelReleaseLookup(): Promise<Map<string, number>> {
  ...
  try {
    const firstPage = await fetchFromDiscogs(
      `/labels/${JAWBOX_LABEL_ID}/releases?page=1&per_page=100&sort=year&sort_order=asc`
    );
    ...
    for (let batchStart = 2; batchStart <= totalPages; batchStart += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        pages.map((p) =>
          fetchFromDiscogs(
            `/labels/${JAWBOX_LABEL_ID}/releases?page=${p}&per_page=100&sort=year&sort_order=asc`
          ).catch(() => null)
        )
      );
