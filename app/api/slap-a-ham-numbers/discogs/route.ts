...
const USER_AGENT = "SlapAHamNumbers/1.0";
const SLAP_A_HAM_NUMBERS_LABEL_ID = 34315;
...
async function getLabelReleaseLookup(): Promise<Map<string, number>> {
  ...
  try {
    const firstPage = await fetchFromDiscogs(
      `/labels/${SLAP_A_HAM_NUMBERS_LABEL_ID}/releases?page=1&per_page=100&sort=year&sort_order=asc`
    );
    ...
    for (let batchStart = 2; batchStart <= totalPages; batchStart += BATCH_SIZE) {
      ...
      const results = await Promise.all(
        pages.map((p) =>
          fetchFromDiscogs(
            `/labels/${SLAP_A_HAM_NUMBERS_LABEL_ID}/releases?page=${p}&per_page=100&sort=year&sort_order=asc`
          ).catch(() => null)
        )
      );
