...
import { JAWBOX_SITE } from "@/lib/music-site";
import {
  DashboardDailyRow,
...
const jawboxArtistsWithImages = jawboxArtists.map((a) => ({
  id: a.id,
  name: a.name,
  imageUrl: JAWBOX_ARTIST_IMAGES[a.id],
}));

export function JawboxDashboardContent() {
  const {
    site,
    isLoading,
    error,
  ...
  return (
    <div className="container py-2">
      <DashboardDailyRow columns={3} description={site.description}>
        ...
        />
        <ArtistOfDayCard
          artists={jawboxArtistsWithImages}
          site={site}
          RemoteImage={SiteRemoteImage}
        />
      </DashboardDailyRow>
