...
import { getJawboxArtistById } from "@/lib/jawbox-artists-data";
...
export function JawboxMemberDetailContent({ memberId }: { memberId: string }) {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const artist = getJawboxArtistById(Number(memberId));
  const artistName = artist?.name?.toLowerCase() ?? "";

  const releases = artist
    ? jawboxReleases.filter(
        (r) => r.artist.toLowerCase() === artist.name.toLowerCase()
      )
    : [];

  const localImage = JAWBOX_ARTIST_IMAGES[Number(memberId)] ?? null;
