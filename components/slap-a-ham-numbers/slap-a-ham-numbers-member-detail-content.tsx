...
import { getSlapAHamNumbersArtistById } from "@/lib/slap-a-ham-numbers-artists-data";
...
export function SlapAHamNumbersMemberDetailContent({ memberId }: { memberId: string }) {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const artist = getSlapAHamNumbersArtistById(Number(memberId));
  const artistName = artist?.name?.toLowerCase() ?? "";

  const releases = artist
    ? slapahamnumbersReleases.filter(
        (r) => r.artist.toLowerCase() === artist.name.toLowerCase()
      )
    : [];

  const localImage = SLAP_A_HAM_NUMBERS_ARTIST_IMAGES[Number(memberId)] ?? null;
