...
import { TOUCH_GO_RECORDS_SITE } from "@/lib/music-site";
import {
  DashboardDailyRow,
...
const touchgorecordsArtistsWithImages = touchgorecordsArtists.map((a) => ({
  id: a.id,
  name: a.name,
  imageUrl: TOUCH_GO_RECORDS_ARTIST_IMAGES[a.id],
}));

export function TouchGoRecordsDashboardContent() {
  const {
    site,
    isLoading,
    error,
  ...
  return (
    <div className="container py-2">
      ...
      <DashboardDailyRow columns={3}>
        ...
        />
        <ArtistOfDayCard
          artists={touchgorecordsArtistsWithImages}
          site={site}
          RemoteImage={SiteRemoteImage}
        />
      </DashboardDailyRow>
