...
import { getTouchGoRecordsArtistById } from "@/lib/touch-go-records-artists-data";
...
export function TouchGoRecordsMemberDetailContent({ memberId }: { memberId: string }) {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const artist = getTouchGoRecordsArtistById(Number(memberId));
  const artistName = artist?.name?.toLowerCase() ?? "";

  const releases = artist
    ? touchgorecordsReleases.filter(
        (r) => r.artist.toLowerCase() === artist.name.toLowerCase()
      )
    : [];

  const localImage = TOUCH_GO_RECORDS_ARTIST_IMAGES[Number(memberId)] ?? null;
