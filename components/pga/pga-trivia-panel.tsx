...
import {
  getDailyPGATriviaQuestions,
  getPGATodayStorageKey,
  type PGATriviaQuestion,
} from "@/lib/pga-trivia-data";
import {
  CheckCircle2,
  XCircle,
  ChevronLeft,
...
function PGATriviaPanelContent() {
  const [quizDate, setQuizDate] = useState<Date | null>(null);
  const [dailyQuestions, setDailyQuestions] = useState<PGATriviaQuestion[]>([]);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [answeredQuestions, setAnsweredQuestions] = useState<AnsweredQuestion[]>([]);
  const [isComplete, setIsComplete] = useState(false);
  const [showYesterday, setShowYesterday] = useState(false);
  const [yesterdayQuestions, setYesterdayQuestions] = useState<PGATriviaQuestion[]>([]);

  useEffect(() => {
    const now = new Date();
    setQuizDate(now);

    const questions = getDailyPGATriviaQuestions(now);
    setDailyQuestions(questions);

    const yesterday = new Date(now);
    yesterday.setDate(yesterday.getDate() - 1);
    setYesterdayQuestions(getDailyPGATriviaQuestions(yesterday));

    const storageKey = getPGATodayStorageKey(now);
  ...
  const handleAnswer = (index: number) => {
    ...
    const storageKey = getPGATodayStorageKey(quizDate!);
  ...
  const handleShare = async () => {
    ...
    const text = `I got ${score}/5 on PGA Numbers trivia; play here! https://majorleaguenumbers.com/pga`;

    if (navigator.share) {
      try {
        await navigator.share({ title: "PGA Numbers Trivia", text });
      } catch (err: any) {
