...
export function TestGithubCommitMembersContent() {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const [search, setSearch] = useState("");

  const filteredArtists = testgithubcommitArtists.filter((artist) =>
  ...
  return (
    <div className="container py-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Showing {filteredArtists.length} of {testgithubcommitArtists.length} artists
      </p>

      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {filteredArtists.map((artist) => {
          const localImage = TEST_GITHUB_COMMIT_ARTIST_IMAGES[artist.id] ?? null;
          return (
