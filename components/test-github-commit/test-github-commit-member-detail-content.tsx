...
import { getTestGithubCommitArtistById } from "@/lib/test-github-commit-artists-data";
...
export function TestGithubCommitMemberDetailContent({ memberId }: { memberId: string }) {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const artist = getTestGithubCommitArtistById(Number(memberId));
  const artistName = artist?.name?.toLowerCase() ?? "";

  const releases = artist
    ? testgithubcommitReleases.filter(
        (r) => r.artist.toLowerCase() === artist.name.toLowerCase()
      )
    : [];

  const localImage = TEST_GITHUB_COMMIT_ARTIST_IMAGES[Number(memberId)] ?? null;
