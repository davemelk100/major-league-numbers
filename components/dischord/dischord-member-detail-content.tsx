...
import { getDischordArtistById } from "@/lib/dischord-artists-data";
...
export function DischordMemberDetailContent({ memberId }: { memberId: string }) {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const artist = getDischordArtistById(Number(memberId));
  const artistName = artist?.name?.toLowerCase() ?? "";

  const releases = artist
    ? dischordReleases.filter(
        (r) => r.artist.toLowerCase() === artist.name.toLowerCase()
      )
    : [];

  const localImage = DISCHORD_ARTIST_IMAGES[Number(memberId)] ?? null;
