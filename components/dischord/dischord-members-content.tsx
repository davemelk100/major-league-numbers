...
export function DischordMembersContent() {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const [search, setSearch] = useState("");

  const filteredArtists = dischordArtists.filter((artist) =>
  ...
  return (
    <div className="container py-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Showing {filteredArtists.length} of {dischordArtists.length} artists
      </p>

      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {filteredArtists.map((artist) => {
          const localImage = DISCHORD_ARTIST_IMAGES[artist.id] ?? null;
          return (
