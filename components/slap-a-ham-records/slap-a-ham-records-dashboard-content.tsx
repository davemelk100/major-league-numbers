...
const slapahamrecordsArtistsWithImages = slapahamrecordsArtists.map((a) => ({
  id: a.id,
  name: a.name,
  imageUrl: SLAP_A_HAM_RECORDS_ARTIST_IMAGES[a.id],
}));

export function SlapAHamRecordsDashboardContent() {
  const {
    site,
    isLoading,
    error,
  ...
  return (
    <div className="container py-2">
      <DashboardDailyRow columns={3} description={site.description}>
        ...
        />
        <ArtistOfDayCard
          artists={slapahamrecordsArtistsWithImages}
          site={site}
          RemoteImage={SiteRemoteImage}
        />
      </DashboardDailyRow>
