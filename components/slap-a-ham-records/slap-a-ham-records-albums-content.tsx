...
type SlapAHamRecordsAlbumGridItem = AlbumGridItem & {
  artist: string;
};

export function SlapAHamRecordsAlbumsContent() {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const [search, setSearch] = useState("");

  const albums: SlapAHamRecordsAlbumGridItem[] = useMemo(
    () =>
  ...
  const getAlbumImage = (album: SlapAHamRecordsAlbumGridItem): string | null => {
  ...
  return (
    <div className="container py-6">
      ...
      <AlbumGrid
        ...
        cacheKeyPrefix="slap-a-ham-records-album-thumb"
        imageFit="contain"
        preferProxy={false}
      />
    </div>
