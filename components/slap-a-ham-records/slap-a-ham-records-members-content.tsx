...
export function SlapAHamRecordsMembersContent() {
  const pathname = usePathname();
  const site = getMusicSiteFromPathname(pathname);
  const [search, setSearch] = useState("");

  const filteredArtists = slapahamrecordsArtists.filter((artist) =>
  ...
  return (
    <div className="container py-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Showing {filteredArtists.length} of {slapahamrecordsArtists.length} artists
      </p>

      <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
        {filteredArtists.map((artist) => {
          const localImage = SLAP_A_HAM_RECORDS_ARTIST_IMAGES[artist.id] ?? null;
          return (
